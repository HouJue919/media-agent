...
- Scores keyframe-to-keyframe motion to estimate video stability.
...
- Summarizes the whole video batch with duration, recommendation, stability, technical, and quality dashboard metrics.
- Supports manual keep, review, and reject decisions inside the video report.
...
- **OpenCV** - blur, exposure, and video stability analysis
...
### v0.2.2 - Video Report Usability

- Added manual keep, review, and reject decisions to the video report.
- Stored video review decisions in browser `localStorage`.
- Added `video_decisions.csv` export.
- Added filename search and recommendation filtering.
- Preserved dashboard metrics and keyframe previews.

### v0.2.3 - Safe Video Organization

- Added `media_agent/video/organize.py`.
- Read `video_decisions.csv` exported from the video report.
- Organized videos into selected keep, review, and reject folders.
- Defaulted to safe copy mode.
- Added optional move mode.
- Prevented overwriting duplicate video file names.
- Generated `video_organize_log.csv`.

### v0.2.4 - Video Stability Scoring

- Added `media_agent/video/stability.py`.
- Estimated keyframe-to-keyframe motion with OpenCV.
- Added `stability_score`, `avg_motion`, `max_motion`, `shaky_frame_count`, and `stability_recommendation`.
- Included stability fields in `video_index.csv` and `video_report.html`.
- Updated video recommendations so severe shake is more likely to require review or become a reject candidate.

### v0.2.5 - Video Dataset Summary Dashboard

- Expanded the video report dashboard for batch-level media review.
- Added total, average, shortest, and longest duration metrics.
- Added recommendation and stability distributions with percentages.
...
The generated video report includes:
...
### Organize Videos from Manual Decisions

After exporting `video_decisions.csv` from the video report:

```bash
python main.py --mode video-organize --video-decisions video_decisions.csv
```

By default, this copies videos into:

```text
organized_videos/
+-- selected_video_keep/
+-- selected_video_review/
+-- selected_video_reject/
+-- video_organize_log.csv
```

Use a custom output folder:

```bash
python main.py --mode video-organize --video-decisions video_decisions.csv --video-organize-output /path/to/organized_videos
```

Use copy mode explicitly:

```bash
python main.py --mode video-organize --video-decisions video_decisions.csv --mode-action copy
```

Use move mode explicitly:

```bash
python main.py --mode video-organize --video-decisions video_decisions.csv --mode-action move
```

There is intentionally no delete mode for videos.
...
The structured video index generated by video mode. It includes video metadata, keyframe counts, aggregated blur and exposure metrics, video stability scores, and video-level recommendations.

### `video_report.html`

A static video quality report with a batch-level dashboard, keyframe previews, search, recommendation filtering, manual decision buttons, and `video_decisions.csv` export.

### `video_decisions.csv`

Exported from the video report after manual review. It stores final user video decisions and can be used by the video organization workflow.
...
The default output directory for safely copied or moved videos based on manual decisions.

### `video_organize_log.csv`

The video organization audit log. It records the original path, target path, user decision, action, and status for each row in `video_decisions.csv`.
